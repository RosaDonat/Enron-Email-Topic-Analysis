...
def main(sc, filename):
    ...
    # The parameter settings for the logistic regression model:
    iterations = [10, 50, 80]
    step_size = [0.1, 1.0]
                    ...
                    for p in range(len(intercept)):
                        for g in range(len(miniBatch)):                        
                            ...
                            for i in range(k):
                                ...
                                metric, area = evaluate(trainingSet, testSet, itr, step, mB, rp, rt, inter)
                            ...
                            param_set = sc.parallelize([(mean_error, itr, step, mB, rp, rt, inter)])
                            try:
    ...
    final_metric, metrics = evaluate(train_data, test_data,itr, step, mB, rp, rt, inter)
    ...
    output = sc.parallelize([('LogisticRegressionWithSGD', errors, result, final_accuracy, areaUnderPRCurve, areaUnderROCCurve, timedelta, timedelta2)])
    output.saveAsTextFile('/Users/rosadonat/temp/project/LogisticRegressionWithSGD_output')    
    sc.stop()
...
def evaluate(train, test, iterations, step, miniBatchFraction, regParam, regType,intercept):
        model = LogisticRegressionWithSGD.train(train, iterations=iterations, step=step, miniBatchFraction=miniBatchFraction, regParam=regParam, regType=regType, intercept=intercept)
