...
def main(sc, filename):
    ...
    #Selecting features from insider data
    ...
    scaler = StandardScaler(withMean = True, withStd = True).fit(needs_scaling)
    # Create new RDD called scaledData that contains scaled values of numerical computed values
    ...
    # The parameter settings for the linear SVM model:
    iterations = [10, 50, 80]
    step_size = [0.1, 1.0]
    reg_param = [0.0, 0.01, 1.0]
    reg_type = [None, 'l1', 'l2']
    ...
    for s in range(len(iterations)):
        for r in range(len(step_size)):
            for m in range(len(reg_param)):
                for n in range(len(reg_type)):
                    for p in range(len(intercept)):
                        itr = iterations[s]
                        step = step_size [r]
                        rp = reg_param[m]
                        rt = reg_type[n]
                        ...
                        for i in range(k):
                            ...
                            metric, area = evaluate(trainingSet, testSet, itr, step, miniBatch, rp, rt, inter)
                        ...
                        param_set = sc.parallelize([(mean_error, itr, step, rp, rt, inter)])
                        try:
    ...
    print("Errors from k-fold cross validation (100 parameter sets using k=5): ", errors)                     
    ...
    final_metric, metrics = evaluate(train_data, test_data,itr, step, miniBatch, rp, rt, inter)
    ...
    output = sc.parallelize([('SVMWithSGD', errors, result, final_accuracy, areaUnderPRCurve, areaUnderROCCurve, timedelta, timedelta2)])
    output.saveAsTextFile('/Users/rosadonat/temp/project/SVMWithSGD_output')    
    sc.stop()
  

##OTHER FUNCTIONS/CLASSES
  
def evaluate(train, test, iterations, step, miniBatchFraction, regParam, regType,intercept):
	model = SVMWithSGD.train(train, iterations=iterations, step=step, miniBatchFraction=miniBatchFraction, regParam=regParam, regType=regType, intercept=intercept)
